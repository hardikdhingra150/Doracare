...
import matplotlib.pyplot as plt#1st task
import seaborn as sns #1st task
import pandas as pd #1st task
import numpy as np # 1st task
from sklearn.model_selection import train_test_split #2nd task
from sklearn.preprocessing import LabelEncoder, StandardScaler #2nd task
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score #2nd task
from sklearn.linear_model import LogisticRegression #2nd task
...
# (baseline training of ml model) 2nd task
# 1. Features and target
X = df.drop(columns=['Label'])   # independent vars
y = df['Label']                  # dependent var (target)
...
# Handle categorical features (basic: convert to dummy vars)
...
# 3. Train a simple classifier (Logistic Regression as baseline)
